...
contract IPFY{
    ...
    function getBuyingMarket(uint256 start, uint256 end) public view returns(IPM[] memory){
        ...
        for(uint256 i=start;i<end;i++){
            ...
            result[i] = IPM(_id,_ip.currenOwner,_ip.buyingPrice,URI[_id],_ip.buyable,_ip.buyable,_ip.time);
        }
        return result;
    }
    function getLendingMarket(uint256 start, uint256 end) public view returns(IPM[] memory){
        ...
        for(uint256 i=start;i<end;i++){
            ...
            IP memory _ip = IPDetails[_id];
            result[i] = IPM(_id,_ip.currenOwner,_ip.lendingPrice,URI[_id],_ip.lendable,_ip.lendable,_ip.time);
        }
        return result;
    }
    function getUserIPs(address _user)public view returns(IPM[] memory){
        ...
        for(uint256 i=0;i<len;i++){
            ...
            IP memory _ip = IPDetails[_id];
            ips[i] = IPM(_id,_ip.currenOwner,_ip.buyingPrice,URI[_id],_ip.lendable,_ip.buyable,_ip.time);
        }
        return ips;
    }
